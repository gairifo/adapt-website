...
// this phase builds and tests it against a Twenty sandbox only.

const { validateLead } = require("./_lib/validate");
const { submitLead } = require("./_lib/twenty-client");
...
module.exports = async function handler(req, res) {
  ...
  try {
    const { personId, companyId } = await submitLead(data);
    ...
    res.status(200).json({ ok: true, personId, companyId });
  } catch (err) {
